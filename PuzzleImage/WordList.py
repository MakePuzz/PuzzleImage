...
class WordList:
    ...
    def draw_wordlist(self, ax):
        ...
        if col_num == 2:
            ...
            if pene_words_count > 0 and peneall:
                ...
                last_w = self.w_num
                ax = self._draw_column(ax, self.words[first_w:last_w], row_spacing, label_x=0.02, y_offset=0.97-row_spacing*(self.row_num)-0.025)
        if col_num == 3:
            # 2nd column
            first_w = self.row_num_at_col_1
            ...
            if pene_words_count > 0:
                ...
                if peneall:
                    ax = self._draw_column(ax, self.words[first_w:last_w], row_spacing, label_x=0.02, y_offset=0.97-row_spacing*(self.row_num)-0.025, separate_space=True)
                if not peneall:
                    col_spacing = (self.w_lens[self.row_num_at_col_1]-3) * 0.05
                    ax = self._draw_column(ax, self.words[first_w:last_w], row_spacing, label_x=0.25+col_spacing, y_offset=0.97-row_spacing*(self.row_num)-0.025, separate_space=True)
        # copyright
        if col_num == 3:
            x_text = 0.95
    ...
    def _get_char_num_per_row(self, row_num, col_num):
        ...
        if col_num == 3:
            char_num_per_row = self.w_lens[self.row_num-1] + self.w_lens[2*self.row_num-1] + self.w_lens[self.w_num-1] + 2 + 4
        ...
        if char_num_per_row > self.char_max_per_row:
            if col_num == 2:
                ...
                while char_num_per_row > self.char_max_per_row:
                    pene_words_count += 1
                    char_num_per_row = self.w_lens[self.row_num-1] + self.w_lens[self.w_num-1-pene_words_count] + 2
            if col_num == 3:
                char_num_at_row_2to3 = (self.char_max_per_row - 2 - self.w_lens[self.row_num-1]) # Subtract the left column from the whole
                peneall = bool(char_num_at_row_2to3 < self.w_lens[self.w_num-1])
                while char_num_per_row > self.char_max_per_row:
                    pene_words_count += 1
                    char_num_per_row = self.w_lens[self.row_num-1] + self.w_lens[2*self.row_num-1] + self.w_lens[self.w_num-1-pene_words_count] + 2 + 4
